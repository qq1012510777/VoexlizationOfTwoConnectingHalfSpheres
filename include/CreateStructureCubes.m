function [structure_] = CreateStructureCubes(X_1, Y_1, Z_1)
    ...
    NumCubes = sizeX * sizeY * sizeZ;
    
    structure_lowest = [];

    % the lowest mesh
    for i = 1:NumCubes / sizeZ
    ...
    for i = 1:sizeZ
        ...
        structure_([(i - 1) * (NumCubes / sizeZ) + 1:i * (NumCubes / sizeZ)], [1:4]) = ...
            structure_lowest + (i - 1) * (sizeX + 1) * (sizeY + 1);
        
        structure_([(i - 1) * (NumCubes / sizeZ) + 1:i * (NumCubes / sizeZ)], [5:8]) = ...
            structure_lowest + (i) * (sizeX + 1) * (sizeY + 1);
